...
for p in range(0,int(sys.argv[2])):
    ...
    for n in range(0,int(sys.argv[3])):
        ...
        for line in open(sys.argv[1], "r"):
            ...
            process = words[4:]
            #print process
            if process.count("P") == p and process.count("N") == n and words[1] == words[2]:
        ...
        if "-r" in o:
            print(idList)
        row_all = row_all + (me + ab + re)
        rows_me = rows_me + me
        rows_ab = rows_ab + ab
        rows_re = rows_re + re
    print "\\ hline " + str(row_all)
    rows = rows + row_all
print("All = %d, All_me = %d, All_ab = %d, All_re = %d"  % (rows, rows_me, rows_ab, rows_re))
